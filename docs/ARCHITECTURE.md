...
게임 세션의 `Component` 참조를 생성자를 통해 전달받아 독점적으로 소유하며, `Input Controller`나 `Presenter`로 상태 참조가 유출되는 것을 물리적으로 차단해야 합니다.
`Engine Event`나 `Internal Event` 등에 반응하여 전체 생명주기와 게임 로직을 통제하는 제어 객체입니다.
...
단일 `Manager`에 책임이 집중되는 것을 방지하기 위해, 다음 3대 기준을 통해 권한을 분리해야 합니다.
1. **Domain Isolation**: 특정 `Component` 집합의 조작에 국한된다면 해당 도메인을 전담하는 `Manager`의 책임입니다.
2. **Phase Isolation**: 특정 페이즈에서만 활성화되며 월드 환경 변화와 생명주기를 같이 한다면 해당 시간을 통제하는 `PhaseManager`의 책임입니다. 페이즈 진입을 위한 조건부 상태 전이 로직 또한 해당 페이즈가 소유해야 합니다.
3. **Global Lifecycle Authority**: 현재 페이즈의 상태와 무관하게 게임 세션 전체를 강제 제어하는 전역 통제 권한은 최상위 `LifecycleManager`의 책임입니다. 상태 전이의 비대칭성을 원칙으로 삼아 권한을 분리해야 합니다.
...
물리적 입력에 대한 1차 검증과 생략된 데이터를 기본값으로 채우는 등의 파싱 연산을 수행하는 것이 원칙입니다. `Feature`의 내부 상태를 직접 조회하여 게임 규칙을 검증하는 행위는 엄격히 금지되며, 모든 판단은 `Manager`에게 맹목적으로 위임해야 합니다.
단, 상태 변이가 발생하지 않는 순수 단순 조회나 로컬 피드백에 한하여
`Manager`를 거치지 않고 `Presenter`를 직접 호출하는 것을 예외적으로 허용합니다.

### 2.7. Presenter

게임 월드 전체의 상태를 갱신하는 시청각 연출은 반드시 `Manager`의 명령을 받아 엔진의 연출 API를 호출해야 합니다.
`Presenter`는 명령어 문법이나 입력 방식 등 파싱 영역에 개입하는 것이 엄격히 금지됩니다. 오직 `Manager`나 `Input Controller`가 확정하여 전달한 순수 데이터를 수동적으로 받아 시청각 연출 분기만을 결정해야 합니다.
단, 특정 유저의 화면이나 채팅창에만 출력되는 에러 메시지 등 입력 맥락이 렌더링에 필수적인 로컬 피드백의 경우,
`Input Controller`로부터 직접 명령을 받을 수 있습니다.
...
* **Presenter Call Delegation**:
  **상태 변이가 발생하여 게임 월드에 영향을 주는가?**를 핵심 기준으로 삼아 호출 주체를 엄격히 분리합니다.
    1. **World Effect**:
       좌표 이동, 체력 갱신 등 게임의 상태 맥락만 필요한 연출. 상태 100% 동기화를 위해 `Manager`가 상태 갱신 직후 직접 호출해야 합니다.
    2. **Local Feedback**:
       에러 텍스트, 조회 화면 등 입력 맥락이 렌더링에 필수적인 연출.
       핵심 로직에 뷰 맥락이 침투하는 것을 막기 위해 결괏값을 받은 `Input Controller`가 직접 호출해야 합니다.
...
* **Flat Data Structure & Flattening**:
  2D 보드판과 같은 다차원 공간은 이중 배열을 배제하고 단일 1차원 배열로 평탄화해야 합니다. 이는 포인터 파편화를 줄이고, 상태 머신에서 순회 좌표 추적을 단일 스칼라 인덱스로 통제하기 위한 핵심 기법입니다.
* **Time-Slicing Initialization Exception**:
  로딩 작업을 프레임 단위로 쪼개어 분산 실행할 경우 해당 궤도는 `Hot Path`로 취급됩니다. 단, 이 루프 내부라 하더라도 룩업 테이블에 영구 캐싱하기 위한 목적의 최초 객체 할당과 외부 API의 조기 종료를 위한 최소한의 방어 분기는 예외적으로 허용됩니다.
* **State Machine Control Flow**:
  실행을 중단하고 다음 틱에 재개해야 하는 분산 반복 작업은, 중단 시 자동 증감식이 스킵되어 상태가 꼬이는 결함을 막기 위해 `for`문 대신 수동 통제가 가능한 `while`문을 강제합니다.
* **Initialization Axioms vs Parameters**:
  객체 초기화에 사용되는 수치라도, 상태의 비어있음을 의미하는 논리적 공리(예: 미초기화 마커 `-1`)는 인라인 상수로 유지합니다. 반면 기획 의도에 따라 변동될 수 있는 밸런스 수치(예: 초기 골드 `0`)는 반드시 외부 상수로 분리하여 격리해야 합니다.
...
3. **Discrete Event-Driven**
    ...
    * 규칙:
      기능이 다른 `Manager` 간의 수평적 직접 호출은 엄격히 금지됩니다. 상태를 변경한 주체는 `Internal Event`를 발행하고 관심 객체가 이를 구독하는 방식으로 수평적 결합도를 완전히 차단해야 합니다. 무거운 엔진 이벤트를 상속받는 것은 금지됩니다.
4. **Atomic Top-Down Direct Call**
    * 대상: 게임 부팅 등 엄격한 순서가 보장되어야 하는 제어
    * 규칙:
      시스템 초기화나 글로벌 리셋 등 전역 생명주기 제어가 필요한 경우, 캡슐화나 이벤트 우회 없이 상위 `Manager`가 하위 `Manager`를 직접 호출해야 합니다.
5. **Phase Transition Ordering**
    * 대상: 게임 페이즈 등 공간적/물리적 환경 셋업을 동반하는 거대 상태 전이
    * 규칙:
      전역 페이즈 전환은 사전 검증, 부수 효과 실행, 상태 변이의 순서를 엄격하게 따릅니다. 환경 셋업이 완벽히 끝난 직후 메인 상태를 갱신해야 다른 시스템의 오작동을 차단할 수 있습니다.

```java
// ❌ DON'T: 상태를 먼저 변경하면 다른 시스템이 셋업 도중에 개입하여 버그를 유발합니다.
public void tryStartGame() {
    if (!hasPlayers()) return;
    this.phase = Phase.STARTING; // 🚨 Zombie State 발생 위험
    setupWorld();
}

// 🟢 DO: 셋업이 완벽히 종료된 후 상태를 갱신하고 이벤트를 브로드캐스팅합니다.
public void tryStartGame() {
    if (!hasPlayers()) return;
    setupWorld();
